...
app = marimo.App(
    width="full",
    layout_file="layouts/konflux-cache-time.grid.json",
)
...
@app.function
def get_pipelineruns(api, namespace, label_selector=None):
    ...
    try:
        ...
        pipelineruns = custom_api.list_namespaced_custom_object(
    ...
    while True:
        for item in pipelineruns.get("items", []):
            yield item
        if cont_key := pipelineruns.get("metadata", {}).get("continue"):
            pipelineruns = custom_api.list_namespaced_custom_object(
...
def plr_time_rec(plr):
    ...
    duration_fields = [
        ("queueTime", "startTime", "finallyStartTime"),
        ("runTime", "finallyStartTime", "completionTime"),
...
@app.function
def plrs_time_table(plrs):
    return pa.Table.from_pylist([plr_time_rec(plr) for plr in plrs])
...
@app.function
def get_prjctl_pr_build_plrs(api, pr_id):
    namespace="konflux-vanguard-tenant"
    plr_labels = ','.join([
        "pipelines.appstudio.openshift.io/type=build",
        "appstudio.openshift.io/application=project-controller",
        "appstudio.openshift.io/component=project-controller",
    ...
    plrs = get_pipelineruns(api, namespace, label_selector=plr_labels)
    plrs = list(plrs)
    return plrs
...
def _(archive_api):
    _pr_number = 641
    _plrs = get_prjctl_pr_build_plrs(archive_api, _pr_number)
    tt = plrs_time_table(_plrs)
    return (tt,)
...
@app.cell
def _(archive_api):
    _cached_pr_number = 642
    _cached_plrs = get_prjctl_pr_build_plrs(archive_api, _cached_pr_number)
    cached_tt = plrs_time_table(_cached_plrs)
    return (cached_tt,)
...
def _(cached_tt, tt):
    ...
    # Load and label data
    _t_tt = _ensure_arrow(tt).append_column("Condition", pa.array(["No Cache (PR 641)"] * len(tt)))
    _t_cached = _ensure_arrow(cached_tt).append_column("Condition", pa.array(["Cached (PR 642)"] * len(cached_tt)))
    ...
    _mean_tt = pa.compute.mean(_filtered.filter(pa.compute.equal(_filtered['Condition'], "No Cache (PR 641)"))['runTime']).as_py()
    _mean_cached = pa.compute.mean(_filtered.filter(pa.compute.equal(_filtered['Condition'], "Cached (PR 642)"))['runTime']).as_py()

    _means_df = pd.DataFrame([
        {'Condition': "No Cache (PR 641)", 'mean_runTime': _mean_tt},
        {'Condition': "Cached (PR 642)", 'mean_runTime': _mean_cached}
    ])
